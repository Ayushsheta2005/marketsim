...
constexpr market_sim::Price stress_price = 10050;
...
StressResult run_stress(std::size_t order_count) {
    ...
    for (std::size_t i = 0; i < order_count; ++i) {
        market_sim::OrderId id = static_cast<market_sim::OrderId>(i + 1);
        book.add_order({id, market_sim::Side::Buy, stress_price, 10, timestamp++});
    }
