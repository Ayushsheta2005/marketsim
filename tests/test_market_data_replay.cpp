...
void check_lobster_book_parse_fails(
    ...
    try {
        static_cast<void>(market_sim::parse_lobster_book_row(line, depth));
...
void check_lobster_validation_fails(
    ...
    try {
        static_cast<void>(market_sim::validate_market_data_replay(
...
void test_lobster_partial_cancel_subtracts_event_quantity() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 6, 3});
...
void test_lobster_partial_cancel_preserves_fifo_position() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 7, 4});
...
void test_lobster_visible_execution_reduces_referenced_order() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Buy, 10060, 6, 3});
...
void test_lobster_cross_trade_does_not_mutate_visible_book() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 10, 3});
...
void test_lobster_unsuccessful_reductions_are_not_counted() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 10, 4});
...
void test_parse_lobster_book_row_one_valid_level() {
    auto row = market_sim::parse_lobster_book_row("10100,25,10090,30", 1);
...
void test_parse_lobster_book_row_preserves_level_order() {
    auto row = market_sim::parse_lobster_book_row(
        "10100,10,10090,20,"
        "10110,30,10080,40,"
        "10120,50,10070,60",
        3
...
void test_parse_lobster_book_row_one_side_empty() {
    auto row = market_sim::parse_lobster_book_row("9999999999,0,10090,30", 1);
...
void test_parse_lobster_book_row_both_sides_empty() {
    auto row = market_sim::parse_lobster_book_row("9999999999,0,-9999999999,0", 1);
...
void test_parse_lobster_book_row_zero_depth_empty_input() {
    auto row = market_sim::parse_lobster_book_row("", 0);
...
void test_validate_market_data_replay_exact_matches() {
    ...
    market_sim::OrderBook book;
    auto summary = market_sim::validate_market_data_replay(messages, expected, book, 1);
...
void test_validate_market_data_replay_exact_one_row_match() {
...
void test_validate_market_data_replay_compares_after_each_event() {
    std::istringstream messages(
        "1,1,1,10,100,1\n"
        "2,3,1,10,100,1\n"
    );
...
void test_validate_market_data_replay_reductions_match() {
    std::istringstream messages(
        "1,1,1,10,100,1\n"
        "2,2,1,4,100,1\n"
        "3,1,2,10,110,-1\n"
...
void test_validate_market_data_replay_price_and_quantity_mismatches() {
    std::istringstream messages(
        "1,1,1,10,100,1\n"
        "2,1,2,5,110,-1\n"
    );
    ...
    CHECK(summary.first_mismatch->side == market_sim::LobsterBookSide::Bid);
    CHECK(summary.first_mismatch->row == 1);
}

void test_validate_market_data_replay_missing_levels() {
    std::istringstream messages(
        "1,5,0,1,100,1\n"
        "2,1,1,10,100,1\n"
    );
...
void test_validate_market_data_replay_first_mismatch_details() {
    ...
    CHECK(summary.first_mismatch.has_value());
    CHECK(summary.first_mismatch->row == 1);
    CHECK(summary.first_mismatch->side == market_sim::LobsterBookSide::Ask);
...
void test_validate_market_data_replay_counts_row_once_and_continues() {
    std::istringstream messages(
        "1,1,1,10,100,1\n"
        "2,2,1,4,100,1\n"
    );
...
void test_validate_market_data_replay_first_mismatch_level_two() {
    std::istringstream messages("1,1,1,10,100,1\n");
    std::istringstream expected(
        "9999999999,0,100,10,9999999999,0,90,5\n"
    );
    market_sim::OrderBook book;
    auto summary = market_sim::validate_market_data_replay(messages, expected, book, 2);
...
void test_validate_market_data_replay_rejects_stream_length_mismatch() {
...
void test_validate_market_data_replay_reports_stream_context() {
    check_lobster_validation_fails(
        "bad\n", "9999999999,0,-9999999999,0\n", 1,
...
void test_validate_market_data_replay_rejects_invalid_arguments() {
    check_lobster_validation_fails("", "", 0, "depth must be greater than zero");

    std::istringstream messages;
    std::istringstream expected;
    market_sim::OrderBook book;
    book.add_order({1, market_sim::Side::Buy, 100, 10, 1});
    bool threw = false;
    try {
        static_cast<void>(market_sim::validate_market_data_replay(
...
market_sim::LobsterTransitionSummary transition_summary(std::string_view messages,
                                                  std::string_view books,
                                                  std::size_t depth) {
    std::istringstream m{std::string(messages)}, b{std::string(books)};
    return market_sim::validate_market_data_transitions(m, b, depth);
}
...
int main() {
    ...
    test_validate_market_data_replay_exact_one_row_match();
    test_validate_market_data_replay_exact_matches();
    test_validate_market_data_replay_compares_after_each_event();
    test_validate_market_data_replay_reductions_match();
    test_validate_market_data_replay_price_and_quantity_mismatches();
    test_validate_market_data_replay_missing_levels();
    test_validate_market_data_replay_first_mismatch_details();
    test_validate_market_data_replay_counts_row_once_and_continues();
    test_validate_market_data_replay_first_mismatch_level_two();
    test_validate_market_data_replay_rejects_stream_length_mismatch();
    test_validate_market_data_replay_reports_stream_context();
    test_validate_market_data_replay_rejects_invalid_arguments();
