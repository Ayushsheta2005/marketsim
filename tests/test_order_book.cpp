...
void test_add_non_crossing_orders() {
    ...
    auto trades1 = book.add_order({1, market_sim::Side::Buy, 10050, 200, 1000});
    auto trades2 = book.add_order({2, market_sim::Side::Sell, 10060, 100, 1001});
...
void test_cancel_existing_order() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 200, 1000});
...
void test_cancel_missing_order_does_not_change_book() {
    ...
    auto trades = book.add_order({1, market_sim::Side::Buy, 10050, 200, 1000});
    bool cancelled = book.cancel_order(999);
...
void test_cancel_already_filled_order() {
    ...
    book.add_order({1, market_sim::Side::Sell, 10060, 100, 1000});

    auto fill_trades = book.add_order({2, market_sim::Side::Buy, 10060, 100, 1001});
    bool cancelled = book.cancel_order(1);
...
void test_crossing_buy_partial_fill() {
    ...
    auto trades = book.add_order({2, market_sim::Side::Buy, 10060, 50, 1001});
...
void test_crossing_buy_full_fill_with_remainder() {
    ...
    auto trades = book.add_order({2, market_sim::Side::Buy, 10060, 150, 1001});
...
void test_crossing_sell_partial_fill() {
    ...
    auto trades = book.add_order({2, market_sim::Side::Sell, 10050, 75, 1001});
...
void test_partial_fill_keeps_positive_resting_quantity() {
    ...
    auto partial_trades = book.add_order({2, market_sim::Side::Buy, 10060, 40, 1001});
    auto remaining_trades = book.add_order({3, market_sim::Side::Buy, 10060, 60, 1002});
...
void test_full_fill_leaves_no_negative_resting_quantity() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 75, 1000});

    auto fill_trades = book.add_order({2, market_sim::Side::Sell, 10050, 75, 1001});
    ...
    auto later_trades = book.add_order({3, market_sim::Side::Sell, 10050, 1, 1002});
...
void test_basic_volume_conservation_with_remainder() {
    ...
    auto trades = book.add_order({2, market_sim::Side::Buy, 10060, 150, 1001});
    auto remainder_trades = book.add_order({3, market_sim::Side::Sell, 10060, 50, 1002});
...
void test_cancel_middle_order_preserves_fifo() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 10, 1000});
    book.add_order({2, market_sim::Side::Buy, 10050, 10, 1001});
    book.add_order({3, market_sim::Side::Buy, 10050, 10, 1002});
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 15, 1003});
...
void test_cancel_head_order_advances_correctly() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 5, 1002});
...
void test_slot_reuse_after_heavy_cancel_churn() {
    ...
    for (int i = 0; i < 1000; ++i) {
        book.add_order({static_cast<market_sim::OrderId>(i), market_sim::Side::Buy, 10050, 10, static_cast<market_sim::Timestamp>(i)});
        book.cancel_order(static_cast<market_sim::OrderId>(i));
    }

    CHECK(book.order_count() == 0);
    CHECK(!book.best_bid().has_value());

    book.add_order({9001, market_sim::Side::Buy, 10050, 5, 1000});
    book.add_order({9002, market_sim::Side::Buy, 10050, 5, 1001});

    auto trades = book.add_order({9003, market_sim::Side::Sell, 10050, 10, 1002});
...
void test_reduce_rejects_invalid_quantity() {
    ...
    CHECK(book.reduce_order(1, 0) == market_sim::ReduceResult::InvalidQuantity);
    CHECK(book.reduce_order(1, -1) == market_sim::ReduceResult::InvalidQuantity);
    CHECK(book.order_count() == 1);
}
...
void test_reduce_missing_order() {
    ...
    CHECK(book.reduce_order(999, 5) == market_sim::ReduceResult::NotFound);
    CHECK(book.order_count() == 0);
}
...
void test_reduce_rejects_quantity_exceeding_resting_quantity() {
    ...
    CHECK(book.reduce_order(1, 11) == market_sim::ReduceResult::ExceedsQuantity);

    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 10, 1001});
...
void test_partial_reduction_subtracts_quantity() {
    ...
    CHECK(book.reduce_order(1, 4) == market_sim::ReduceResult::Reduced);
    CHECK(book.order_count() == 1);

    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 6, 1001});
...
void test_partial_reduction_preserves_fifo_position() {
    ...
    auto trades = book.add_order({100, market_sim::Side::Sell, 10050, 7, 1002});
...
void test_exact_reduction_removes_order() {
    ...
    CHECK(book.reduce_order(1, 10) == market_sim::ReduceResult::Removed);
    CHECK(book.order_count() == 0);
    CHECK(!book.best_bid().has_value());
    CHECK(book.reduce_order(1, 1) == market_sim::ReduceResult::NotFound);
}

void test_empty_book_snapshot_is_empty() {
    const market_sim::OrderBook book;
...
void test_snapshot_depth_zero_is_empty() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 10, 1000});
    book.add_order({2, market_sim::Side::Sell, 10060, 20, 1001});
...
void test_snapshot_aggregates_orders_at_same_price() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 10, 1000});
    book.add_order({2, market_sim::Side::Buy, 10050, 15, 1001});
...
void test_snapshot_orders_bids_descending() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 10, 1000});
    book.add_order({2, market_sim::Side::Buy, 10070, 20, 1001});
    book.add_order({3, market_sim::Side::Buy, 10060, 30, 1002});
...
void test_snapshot_orders_asks_ascending() {
    ...
    book.add_order({1, market_sim::Side::Sell, 10070, 10, 1000});
    book.add_order({2, market_sim::Side::Sell, 10050, 20, 1001});
    book.add_order({3, market_sim::Side::Sell, 10060, 30, 1002});
...
void test_snapshot_depth_truncates_each_side_independently() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10030, 10, 1000});
    book.add_order({2, market_sim::Side::Buy, 10020, 20, 1001});
    book.add_order({3, market_sim::Side::Buy, 10010, 30, 1002});
    book.add_order({4, market_sim::Side::Sell, 10040, 40, 1003});
    book.add_order({5, market_sim::Side::Sell, 10050, 50, 1004});
    book.add_order({6, market_sim::Side::Sell, 10060, 60, 1005});
...
void test_snapshot_omits_level_after_final_order_cancelled() {
    ...
    book.add_order({1, market_sim::Side::Buy, 10050, 10, 1000});
    book.add_order({2, market_sim::Side::Buy, 10040, 20, 1001});
