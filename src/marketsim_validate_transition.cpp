...
std::string_view side_name(market_sim::LobsterBookSide side) {
    return side == market_sim::LobsterBookSide::Bid ? "bid" : "ask";
}

void print_level(const std::optional<market_sim::LevelSnapshot>& level) {
    if (!level) {
        std::cout << "missing";
        return;
    }
...
void print_summary(
    ...
    std::size_t depth,
    const market_sim::LobsterTransitionSummary& summary
) {
    ...
    if (summary.first_mismatch) {
        const market_sim::LobsterTransitionMismatch& mismatch =
